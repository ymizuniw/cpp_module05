...
int test_02(void) {
  ...
  // Robotomy exception test 72, 45
  {
    ...
    std::cout << "==Bureaucrat::executeForm(RobotomyRequestForm) Test=="
              << std::endl;
    br1.executeForm(rf1);
  }
  // PresidentialPardonForm Test
  {
    ...
    PresidentialPardonForm pf2("Trilian");
    std::cout << "==Copy Constructor Test==" << std::endl;
    Bureaucrat br1("br1", 1);
    pf2.beSigned(br1);
    PresidentialPardonForm pf3(pf2);
    if (!(pf2.getSigned() == pf3.getSigned() &&
          pf2.getGradeToExec() == pf3.getGradeToExec() &&
          pf2.getGradeToSign() == pf3.getGradeToSign())) {
      std::cout << "PresidentialPardonForm Copy Constructor Test failed!"
                << std::endl;
      return (1);
    }
    std::cout << "==Copy Assignment Operator Test==" << std::endl;
    PresidentialPardonForm pf4("pf4");
    pf4 = pf2;
    if (!(pf2.getSigned() == pf4.getSigned() &&
          pf2.getGradeToExec() == pf4.getGradeToExec() &&
          pf2.getGradeToSign() == pf4.getGradeToSign())) {
      std::cout
          << "PresidentialPardonForm Copy Assignment Operator Test failed!"
          << std::endl;
      return (1);
    }
    std::cout << "==Destructor Test==" << std::endl;
  }
  // PresidentialPardonForm Basic Function Test
  {
    ...
    try {
      ...
      std::cout << e.what() << std::endl;
      std::cout << "PresidentialPardonForm Execution Test failed!" << std::endl;
      return (1);
    }
    std::cout << "==execute() Test==" << std::endl;
    br1.executeForm(pf1);
  }
  // PresidentialPardonForm Exception Test
  {
    int                    exception_occurred = 0;
    PresidentialPardonForm pf1("pf1");
    Bureaucrat             br1("br1", 6);
    try {
      pf1.execute(br1);
    } catch (const std::exception& e) {
      exception_occurred = 1;
      if (std::string(e.what()) !=
        ...
        std::cerr << "Unexpected Exception is thrown" << std::endl;
        std::cerr << "PresidentialPardonForm Exception Test failed!"
                  << std::endl;
        return (1);
      }
      std::cout << e.what() << std::endl;
    ...
    if (exception_occurred == 0) {
      std::cerr << "Exception is not thrown" << std::endl;
      std::cerr << "PresidentialPardonForm Exception Test failed!" << std::endl;
      return (1);
    }
  }
